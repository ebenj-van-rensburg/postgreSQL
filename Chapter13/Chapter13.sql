...
FROM 'D:\bootcamp\SQL\myCode\Chapter13\crime_reports.csv'
...
-- Listing 13-5: Using regexp_match() to find the second date, note that the result includes an unwanted hyphen
SELECT crime_id,
       regexp_match(original_text, '-\d{1,2}\/\d{1,2}\/\d{1,2}')
FROM crime_reports;

-- Listing 13-6: Using a capture group to return only the date eliminates the hyphen
SELECT crime_id,
       regexp_match(original_text, '-(\d{1,2}\/\d{1,2}\/\d{1,2})')
FROM crime_reports;
...
FROM 'D:\bootcamp\SQL\myCode\Chapter13\sotu-1946-1977.csv'
