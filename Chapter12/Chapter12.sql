...
WHERE emp_id IN (
    SELECT id
    FROM retirees);
-- Checking whether values exist (returns all rows from employees if the expression evaluates as true)
SELECT first_name, last_name
FROM employees
WHERE EXISTS (
    SELECT id
...
WHERE (p0010001 - us_median_pop)
       BETWEEN -1000 AND 1000;
-- Cross tabulations: install the crosstab() function via the tablefunc module
...
FROM 'D:\bootcamp\SQL\source\practical-sql-main\Chapter_12\ice_cream_survey.csv'
...
FROM 'D:\bootcamp\SQL\source\practical-sql-main\Chapter_12\temperature_readings.csv'
WITH (FORMAT CSV, HEADER);
